...
const _marketLoop = () => {
    ...
    if (window._isPaused) { _marketTimeout=setTimeout(_marketLoop,500); return; }
    fetchMarket();
    ...
    assetDatabase.forEach(a => {
        ...
        if (isBond) {
            ...
            if (Math.random() < 0.01) {
                ...
                showToast(petP==='frog' ? ` Crash en ${a.symbol}! (Rana protegió)` : ` Crash en ${a.symbol}! -${(crashAmt*100).toFixed(1)}%`);
                if (state.portfolio.has(a.symbol)) {
                    if (petP!=='frog') { if(typeof changePetHealth==='function') changePetHealth(-3); }
                    else { const isDiamond=PET_DEFS[state.currentPet]?.diamond; if(!isDiamond&&typeof changePetHealth==='function') changePetHealth(-1); }
                }
            } else {
                next = Math.max(0.000001, oldPrice * (1 + (Math.random()*0.05-0.025)));
            }
        }
...
const renderMarket = () => {
    ...
    refs.marketList.innerHTML = assets.map(a => {
        ...
        <div class="asset-row ${hlClass}">
            ...
            <div class="asset-price">${(()=>{const d=typeof window.applyPetBuyModifiers==='function'?window.applyPetBuyModifiers(a.price):a.price;return d<a.price?`<span style="text-decoration:line-through;opacity:.5;font-size:.8em;">${fmt(a.price)}</span> <span style="color:#52b788;font-weight:700;">${fmt(d)}</span>`:fmt(a.price);})()} ${futureTag}</div>
...
window.setNewGoal = () => {
    ...
    if (!isNaN(nuevaMeta) && nuevaMeta > 0) {
        state.metaFinanciera = nuevaMeta; state.metaAlcanzada = false;
        saveGame(); window.checkInvestmentGoal(); showToast('Nueva meta: ' + fmt(nuevaMeta));
    }
};
...
window.checkLevelUp = () => {
    ...
    if (lvl > window._lastLevel && window._lastLevel > 0) {
        const table = getLevelTable(); const rewards = table[lvl-1]?.[1] || [];
        showLevelUpModal(lvl, rewards);
        rewards.forEach(r => {
            const food = foodDatabase?.find(f=>f.id===r.id); if (!food) return;
            const ex = state.inventory.get(r.id) || {...food, qty:0};
            state.inventory.set(r.id, {...ex, qty:(ex.qty||0)+r.qty});
        });
        saveGame(); if (typeof renderInventory==='function') renderInventory();
    }
    window._lastLevel = lvl; updateLevelUI();
};
const updateLevelUI = () => {
...
const showLevelUpModal = (level, rewards) => {
    ...
    const rewardHtml = rewards.map(r => {
        ...
        return `<div style="display:flex;align-items:center;gap:8px;background:#FFF5BA;border-radius:10px;padding:6px 12px;"><img src="../assets/food/${r.id}.png" style="width:32px;height:32px;object-fit:contain;"><span style="color:#CBA6F7;font-weight:700;font-size:0.9rem;">${food?.name||r.id} x${r.qty}</span></div>`;
    }).join('');
    modal.innerHTML = `<div class="modal-box" style="max-width:360px;text-align:center;border:3px solid #CBA6F7;box-shadow:5px 5px 0 #FFF5BA;">
        ...
        <button onclick="document.getElementById('modal-levelup').style.display='none'" style="background:#CBA6F7;color:white;border:none;padding:10px 24px;border-radius:12px;font-weight:700;font-family:'Poppins',sans-serif;cursor:pointer;font-size:0.9rem;">Continuar</button>
    </div>`;
...
const recordChartTick = () => {
    ...
    state.market.forEach((asset,sym) => {
        ...
        const arr=chartHistory.get(sym); arr.push({price:asset.price,time:now});
        if (arr.length>CHART_MAX_POINTS) arr.shift();
    });
    if (chartSymbol&&document.getElementById('modal-chart')?.style.display!=='none') { drawChart(); updateChartUI(); }
};
window.openChart=(symbol)=>{
    chartSymbol=symbol; document.getElementById('modal-chart').style.display='flex';
    buildChartSelector(symbol); updateChartUI(); drawChart(); setTimeout(initChartTooltip,50);
};
window.closeChart=()=>{ document.getElementById('modal-chart').style.display='none'; };
const buildChartSelector=(selected)=>{
    const el=document.getElementById('chart-selector'); if(!el)return;
...
const updateChartUI=()=>{
    ...
    const priceEl=document.getElementById('chart-price-display'); const buyBtn=document.getElementById('chart-buy-btn');
    if(discPrice<asset.price){priceEl.innerHTML=`<span style="text-decoration:line-through;opacity:.5;font-size:.8em;">${fmt(asset.price)}</span> <span style="color:#52b788;">${fmt(discPrice)}</span>`;}
    ...
    if(arr.length>=2){
        ...
        const st="background:#f9f0ff;border-radius:10px;padding:5px 10px;font-size:10px;font-family:'Poppins',sans-serif;flex-shrink:0;min-width:70px;";
        statsEl.innerHTML=`<div style="${st}"><span style="color:#CBA6F7;opacity:.7;">Cambio 5min</span><br><strong style="color:${isUp?'#52b788':'#FFB6C1'};">${isUp?'▲':'▼'} ${Math.abs(change).toFixed(2)}%</strong></div><div style="${st}"><span style="color:#CBA6F7;opacity:.7;">Mínimo</span><br><strong style="color:#CBA6F7;">${fmt(minP)}</strong></div><div style="${st}"><span style="color:#CBA6F7;opacity:.7;">Máximo</span><br><strong style="color:#CBA6F7;">${fmt(maxP)}</strong></div><div style="${st}"><span style="color:#CBA6F7;opacity:.7;">Actual</span><br><strong style="color:#CBA6F7;">${fmt(last)}</strong></div>`;
...
const drawChart=()=>{
    ...
    const ctx=canvas.getContext('2d'); const arr=chartHistory.get(chartSymbol)||[];
    const W=canvas.offsetWidth||600,H=canvas.offsetHeight||200;
    canvas.width=W*window.devicePixelRatio; canvas.height=H*window.devicePixelRatio;
    ctx.scale(window.devicePixelRatio,window.devicePixelRatio); ctx.clearRect(0,0,W,H);
    ...
    const pad={top:16,right:16,bottom:24,left:60},cW=W-pad.left-pad.right,cH=H-pad.top-pad.bottom;
    const toX=i=>pad.left+(i/(arr.length-1))*cW,toY=p=>pad.top+(1-(p-minP)/range)*cH;
    ...
    grad.addColorStop(0,isUp?'rgba(178,242,187,0.5)':'rgba(255,182,193,0.5)');grad.addColorStop(1,'rgba(255,255,255,0)');
    ctx.beginPath();ctx.moveTo(toX(0),toY(prices[0]));prices.forEach((p,i)=>{if(i>0)ctx.lineTo(toX(i),toY(p));});
    ...
    if(arr.length>=2){const st=new Date(arr[0].time).toLocaleTimeString(),et=new Date(arr[arr.length-1].time).toLocaleTimeString();ctx.fillStyle='#CBA6F7';ctx.font='9px Poppins, sans-serif';ctx.textAlign='left';ctx.fillText(st,pad.left,H-6);ctx.textAlign='right';ctx.fillText(et,pad.left+cW,H-6);}
};
window.chartBuySelected=()=>{ if(!chartSymbol)return; buy(chartSymbol); updateChartUI(); };
const initChartTooltip=()=>{
    const canvas=document.getElementById('chart-canvas'),tooltip=document.getElementById('chart-tooltip');
    if(!canvas||!tooltip)return;
    canvas.addEventListener('mousemove',(e)=>{
        const arr=chartHistory.get(chartSymbol)||[]; if(arr.length<2)return;
        const rect=canvas.getBoundingClientRect(),mx=e.clientX-rect.left,W=rect.width;
        const pad={left:60,right:16},cW=W-pad.left-pad.right;
        const idx=Math.round(((mx-pad.left)/cW)*(arr.length-1));
        const pt=arr[Math.max(0,Math.min(idx,arr.length-1))]; if(!pt)return;
