...
// ============================================================
window.buy = (symbol) => {
    if (state.portfolio.has(symbol)) { showToast('Ya tienes este activo.'); return; }
...
window.sellFromPortfolio = (symbol) => {
    ...
    if (realProfit > 0) {
        ...
        showToast(`Vendiste ${symbol}! +${fmt(realProfit)}${x2tag}${petMsg} +${gain}`);
        ...
        showToast(`Vendiste ${symbol}. ${fmt(Math.abs(realProfit))}${x2tag}${petMsg} -${loss}`);
        logEvent('venta', `Vendiste ${symbol} con PÉRDIDA`, `${fmt(Math.abs(realProfit))}${x2tag}${petMsg}`);
    } else {
        showToast(`Vendiste ${symbol}${petMsg}`);
    }
    updateUI();
    checkGameOver();
    checkSectorBonus();
...
window.doDeposit = (amount, costPct) => {
    ...
    if (state.saludMascota <= needed) {
        const warn = document.getElementById('deposit-warning');
        if (warn) { warn.textContent = `Tu mascota necesita más de ${needed}% de salud para este depósito. Salud actual: ${state.saludMascota}%`; warn.style.display='block'; }
        return;
    }
    state.monedas += amount;
    changePetHealth(-costPct);
    closeDeposit();
    showToast(`Depositaste ${amount.toLocaleString()} (-${costPct}% )`);
...
window.openFoodShop = () => {
    ...
    grid.innerHTML = foodDatabase.map(f => {
        ...
        <div class="food-item ${catClass[f.cat]}">
            ...
            ${f.health ? `<span class="food-hp"> +${f.health}</span>` : ''}
            ${inflTag}
...
const useFoodOnPet = () => {
    ...
    switch(item.cat) {
        case 'verdura':
            ...
            showToast(`Diste ${item.name} a tu mascota! +${item.health}`);
            logEvent('mascota', `Diste ${item.name} a tu mascota`, `Salud +${item.health}`);
            break;
        case 'fruta':
        ...
        case 'misc':
            changePetHealth(item.health);
            showToast(` Diste ${item.name}! +${item.health}`);
            break;
    }
...
const checkSectorBonus = () => {
    ...
    counts.forEach((count, type) => {
        ...
        if (count < BONUS_THRESHOLD && state.sectorBonus.get(type)) {
            ...
            showToast(`Perdiste el bono de ${type}: ya no tienes 3 acciones en ese sector.`);
        }
    });
...
const triggerMarketEventWarning = () => {
    ...
    showToast(evt.type === 'crash'
        ? ` ${evt.sector} — evento de CAÍDA en ${warningTime}s!`
        : ` ${evt.sector} — evento de SUBIDA en ${warningTime}s!`
    );
    logEvent('evento', ` Alerta: evento ${evt.type === 'crash' ? 'CAÍDA' : 'SUBIDA'} en ${evt.sector}`, `Se aplicará en ${warningTime}s`);
...
window.openTutorial = () => {
    ...
    const tryOpen = () => {
        if (typeof TUTORIAL_STEPS !== 'undefined') {
            ...
            showToast(' Tutorial no disponible. Recarga la página e inténtalo de nuevo.');
        }
    };
    tryOpen();
};
